...
begin
   ...
   for Board_Index in Board_Base_Adress'Range loop
      Port_IO.Out_Byte(Address => Board_Base_Adress(Board_Index) + 3,
                       Data    => 16#99#);

      Port_IO.Out_Byte(Address => Board_Base_Adress(Board_Index) + 7,
                       Data    => 16#82#);

      for Port_Offset_Index in Port_Offset_Array'Range loop
         Port_IO.Out_Byte(Address => Board_Base_Adress(Board_Index)
                          + Port_Offset_Array(Port_Offset_Index),
                          Data    => 0);
      end loop;
   end loop;
end Motors;
