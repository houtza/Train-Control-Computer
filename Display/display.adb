...
package body Display is
   ...
   -- Modified by John McCormick, May 2008
   ...
   type Message_Type is (Train_Name, Train_Direction,
      ...
      record
         ...
         case Which is
            ...
            when Train_Throttle =>
               Throttle       : Percent;
            when Train_Blocks =>
               Size           : Block_Range;    -- A list of blocks
               Blocks         : Block_Array;
            when Train_Stop =>
      ...
      loop
         ...
         case Message.Which is
            ...
            when Train_Blocks =>
               ...
               for Index in 1 .. Message.Size loop
                  Put (Item  => Integer (Message.Blocks (Index)),
                       Width => 3);
               end loop;
               -- Fill the rest of the line with blanks
               Put ((Max_List - Message.Size) * "   ");
   ...
   ----------------------------------------------------------------------------
   procedure Put (Train  : in Trains.Train_ID;
                  Blocks : in Layout.Block_Array) is
      Message : Message_Rec (Which => Train_Blocks);
   begin
      Message.Train  := Train;
      Message.Size   := Blocks'Length;
      Message.Blocks (1 .. Blocks'Length) := Blocks;
      Message_Buffer.Put (Message);
   end Put;
